...
/**
 * ממיר RichTextValue של Google Sheets למקטעים בטוחים לאתר.
 * נשמרים: צבע, מודגש, נטוי וקו תחתון.
 */
function richTextValueToSegments(richTextValue) {
  if (!richTextValue) {
    return [];
  }
  ...
  return runs.map(function(run) {
    ...
    return {
      ...
      color: normalizeHexColor(
        style ? style.getForegroundColor() : ''
      ),
      bold: Boolean(
        style && style.isBold()
      ),
      italic: Boolean(
        style && style.isItalic()
      ),
      underline: Boolean(
...
/**
 * מחזיר צבע Hex בטוח או מחרוזת ריקה.
 */
...
function readCoordinatorMessages(
  ...
  const richTextRows = range.getRichTextValues();
